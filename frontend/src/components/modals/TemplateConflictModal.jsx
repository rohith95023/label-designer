...
export default function TemplateConflictModal({ 
  ...
  return createPortal(
    <div className="fixed inset-0 z-[99999] flex items-center justify-center bg-slate-900/40 backdrop-blur-sm px-4">
      <motion.div 
        initial={{ opacity: 0, scale: 0.95, y: 20 }}
        animate={{ opacity: 1, scale: 1, y: 0 }}
        exit={{ opacity: 0, scale: 0.95, y: 20 }}
        className="bg-white rounded-[32px] shadow-3xl w-full max-w-md overflow-hidden border border-white/20"
      >
        {/* Header with Icon */}
        <div className="pt-8 px-8 pb-4 flex flex-col items-center">
          <div className="w-16 h-16 rounded-3xl bg-amber-50 flex items-center justify-center mb-6 shadow-sm border border-amber-100">
          ...
          <p className="text-[13px] font-medium text-slate-500 mt-2 text-center leading-relaxed">
        ...
        <div className="p-6 pt-2 flex flex-col gap-3">
          ...
          <button
            ...
            className="group flex items-center gap-4 p-4 rounded-2xl bg-slate-50 hover:bg-red-50 border border-slate-200/50 hover:border-red-200 transition-all text-left"
          >
            <div className="w-10 h-10 rounded-xl bg-white flex items-center justify-center text-slate-400 group-hover:text-red-500 shadow-sm border border-slate-100 transition-colors">
              <span className="material-symbols-outlined text-[20px]">layers_clear</span>
            </div>
            <div className="flex-1">
              <span className="block text-[13px] font-bold text-slate-800">{replaceLabel}</span>
              <span className="block text-[10px] font-medium text-slate-500 mt-0.5">{replaceDescription}</span>
            </div>
          ...
          <button
            onClick={onClose}
            className="mt-2 w-full py-3 rounded-2xl text-[11px] font-black uppercase tracking-widest text-slate-400 hover:text-slate-600 transition-all text-center"
          >
